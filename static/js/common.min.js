...
// 注册
var lading_html = '';
lading_html += '<div class="landing_window">';
lading_html += '<div class="landing_box">';
lading_html += '<div class="close"></div>';
lading_html += '<div class="login_error">';
lading_html += '<div><i></i><span>账号不存在</span></div>';
lading_html += '</div>';
lading_html += '<input type="text" class="landing_email" placeholder="邮箱地址">';
lading_html += '<input type="password" class="landing_pass" placeholder="密码">';
lading_html += '<a href="javascript:;" class="landing_btn">登录</a>';
lading_html += '<div class="landing_flex"><a href="./密码找回.html">忘记密码</a><a href="./注册.html">新用户注册</a></div></div></div>';
$('body').on('click', '#click_on_lading', function(){
    $('body').append(lading_html);
})
$('body').on('click', '.landing_window .close', function(){
    $('.landing_window').remove();
});
$('body').on('click', '.landing_btn', function(){
    var $landing_email = $('.landing_email').val();
    var $landing_pass = $('.landing_pass').val();
    var emailReg = /[\w-.]+@[\w-]+(.[\w_-]+)+/;
    if(!$landing_email){
        $('.landing_email').focus();
        $('.landing_window .login_error').find('span').html('邮箱地址不能为空')
        $('.landing_window .login_error div').show();
        return ;
    }
    if(!emailReg.test($landing_email)){
        $('.landing_email').focus();
        $('.landing_window .login_error').find('span').html('输入邮箱格式错误')
        $('.landing_window .login_error div').show();
        return ;
    }
    if(!$landing_pass){
        $('.landing_pass').focus();
        $('.landing_window .login_error').find('span').html('输入的密码不能空')
        $('.landing_window .login_error div').show();
        return ;
    }
    $.ajax({
        url: '',
        type: 'post',
        data: {username: $landing_email, pwd: $landing_pass},
        success: function(data){
            if(data.status === 0){
                $('.landing_window .login_error').find('span').html('账号不存在或密码错误')
                $('.landing_window .login_error div').show(); 
            }else{
                location.href = '/'
            }
